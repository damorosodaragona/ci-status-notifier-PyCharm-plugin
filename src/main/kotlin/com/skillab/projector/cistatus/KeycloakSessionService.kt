    ...
    fun attemptAutoLoginInBackground(baseUrl: String): Boolean {
        ...
        try {
            ...
            ApplicationManager.getApplication().invokeAndWait {
                browserHolder[0] = JBCefBrowser(baseUrl)
                val b = browserHolder[0] ?: return@invokeAndWait
                val script = autoLoginScript(baseUrl, username, password)
                b.cefBrowser.client.addLoadHandler(object : CefLoadHandlerAdapter() {
            ...
            if (toDispose != null) {
                ApplicationManager.getApplication().invokeLater({ Disposer.dispose(toDispose) }, ModalityState.any())
            }
